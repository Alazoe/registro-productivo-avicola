// ============================================================
// PUNTO DE ENTRADA — doGet maneja JSONP para todas las acciones
// ============================================================
function doGet(e) {
  // Si hay parámetro ?callback= → respuesta JSONP (llamadas desde el frontend)
  if (e && e.parameter && e.parameter.callback) {
    return manejarJSONP(e);
  }
  // Si no hay callback → servir la interfaz HTML
...
function manejarJSONP(e) {
  ...
  try {
    ...
    switch (accion) {
      case 'obtenerLotes':
        resultado = obtenerLotesActivos();
        break;

      case 'obtenerUltimoDia':
        resultado = obtenerUltimoDia(e.parameter.nombreHoja);
        break;

      case 'guardarDatos':
        const datos = JSON.parse(e.parameter.datos);
        resultado = guardarDatos(datos);
        break;
      ...
      case 'obtenerGraficos':
        resultado = obtenerDatosGraficos(e.parameter.nombreHoja);
        break;

      default:
        resultado = { exito: false, mensaje: 'Acción desconocida: ' + accion };
    }
  } catch (error) {
    Logger.log('Error en manejarJSONP: ' + error.message);
    resultado = { exito: false, mensaje: 'Error del servidor: ' + error.message };
  }
...
function guardarDatos(datos) {
  try {
    ...
    if (!ws) return { exito: false, mensaje: 'No se encontró la hoja ' + nombreHoja };

    // Encontrar primera fila vacía a partir de la fila 9
    let ultimaFila = 9;
    ...
    if (!fechaNac) return { exito: false, mensaje: 'No se encontró configuración del lote' };

    const edadSemanas = Math.floor((fecha - fechaNac) / (7 * 24 * 60 * 60 * 1000));

    // Curva de postura esperada
    ...
    ws.getRange(ultimaFila, 17).setNumberFormat('0.000');
    ws.getRange(ultimaFila, 18, 1, 3).setNumberFormat('0.0%');
...
function obtenerUltimoDia(nombreHoja) {
  try {
    ...
    // Sin registros → usar aves de configuración
...
// ============================================================
// UTILIDAD — generar objeto CURVAS para copiar al frontend
// ============================================================
