...
// Code.gs completo con endpoint getDashboard para AVIVET Monitor
// Estructura: 21 columnas con clasificación por tamaño y calidad
// ═══════════════════════════════════════════════════════════════════════

function doGet(e) {
  // ── Endpoint para el dashboard externo ──────────────────────────────
  if (e.parameter.action === 'getDashboard') {
    return getDashboard();
  }

  // ── Formulario de registro (comportamiento original) ─────────────────
...
// ═══════════════════════════════════════════════════════════════════════
// ENDPOINT DASHBOARD — CLARITA / GH (21 columnas)
...
//           17:Kg/Ave 18:%Postura 19:%Esperado 20:Diferencia 21:Obs
// CONFIGURACIÓN: filas 10-14, col2=nombre, col3=fechaNac,
//                col4=lineaGenetica, col5=avesInicio, col6=estado
// ═══════════════════════════════════════════════════════════════════════
...
function getDashboard() {
  try {
    ...
    // ── Leer lotes activos desde CONFIGURACIÓN ──────────────────────
    ...
    const lotes = hojas.map(hoja => {
        ...
        for (let i = 10; i <= 14; i++) {
          if (wsConfig.getRange(i, 2).getValue() === nombre) {
            ...
            if (fechaNac instanceof Date) {
              semanaVida = Math.floor((hoy - fechaNac) / (7 * 86400000));
            }
            break;
          }
        }
      ...
      const kgAlimUltimo = parseFloat(ultima[4]) || 0;  // col 5 → idx 4
      const nHuevosUltimo = parseInt(ultima[5])  || 0;  // col 6 → idx 5
      const gAlimPorHuevo = nHuevosUltimo > 0
        ? Math.round((kgAlimUltimo * 1000) / nHuevosUltimo * 10) / 10
        : null;

      // Últimos 28 días para gráfico
